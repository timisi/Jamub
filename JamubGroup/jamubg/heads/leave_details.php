	...
	// <!-- FOR THE USERs DATA -->
		...
		if ($status === '2') {
			...
			if ($result) {
				...
				echo "<script>alert('STAFF EMAIL IS INVALID. APPRAISAL APPLICATION REJECTED BUT NO EMAIL SENT');</script>";
                echo "<script type='text/javascript'> document.location = 'index.php'; </script>";
				}
				
			} else {
				echo "<script>alert('YOUR EMAIL IS INVALID. APPRAISAL APPLICATION REJECTED BUT NO EMAIL SENT');</script>";
                echo "<script type='text/javascript'> document.location = 'index.php'; </script>";
			}
			} else{
				die(mysqli_error());
